...
<body>
    ...
    <div class="container mt-5">
        <div class="row">
            <div class="col-11 mx-auto text-center h1">
              أهلًا بك عميلنا العزيز
            </div>
            </div>
            </div>
            ...
            <div class="col-11">
            <div class="h3 text-center mb-5">نتيجة تحليل</div>
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" class="mt-4" method="post">
                <div class="form-group text-right">
                    ...
                    <input type="text"  class="form-control" name="t_ran_code" required>
...
if (isset($_POST['t_ran_code'])) {
    ...
    $t_ran_code = $_POST['t_ran_code'];

    $sql = "SELECT * FROM test WHERE t_ran_code = '$t_ran_code' AND t_result IS NOT NULL";
    $stmt = sqlsrv_query( $conn, $sql);
    $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC);
    
   $date = $row['t_insert_date'];   
   $formatted_date = date("F j, Y, g:i a", strtotime($date));
   
   $patient_id = $row['c_id']; 
   $patient_tests = $row['t_user_tests']; 
   $patient_price = $row['t_total_price']; 
   $patient_reg = $formatted_date; 
   $patient_result = $row['t_result'];
 ...
 //  echo $patient_result;
    ...
    if ($row) {
        echo "<meta http-equiv='refresh' content='0; url=./UserResult.php'>;";
    }else{
        echo "<div class='row mt-5 mb-5 mx-auto text-center'> <div class='col-12'><a class='btn btn-secondary' href='./result.php'>عفوًا لم يتم إصدا نتيجة التحليل بعد أو أن قد تم إدخال كود تحليل خاطيء, الرجاء المحاولة لاحقًا</a></div></div>";
    }

}
echo "<hr>";
echo "<div class='row mt-5 mb-5 mx-auto text-center'> <div class='col-12'><a class='btn btn-secondary' href='../Admin0/Admin0.php'>عودة</a></div></div>";

// export result data :
$_SESSION['patient_id'] = $patient_id;
$_SESSION['patient_tests'] = $patient_tests;
$_SESSION['patient_price'] = $patient_price;
$_SESSION['patient_reg'] = $formatted_date;
$_SESSION['patient_result'] = $patient_result;
