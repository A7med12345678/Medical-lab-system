...
//$sql = "SELECT worker.w_name, login_record.w_id, login_record.w_login_time FROM worker INNER JOIN login_record ON worker.w_id = login_record.w_id WHERE DATEPART(hour, w_login_time) >= 9 AND DATEPART(minute, w_login_time) > 0";
$sql = "SELECT worker.w_name, login_record.w_id, login_record.w_login_time
FROM worker
INNER JOIN login_record ON worker.w_id = login_record.w_id
WHERE MONTH(w_login_time) = MONTH(GETDATE())
AND YEAR(w_login_time) = YEAR(GETDATE())
AND DATEPART(hour, w_login_time) >= 9
AND DATEPART(minute, w_login_time) > 0
";
$params = array(1, "some data");

$stmt = sqlsrv_query( $conn, $sql, $params);

$date = $row['w_login_time'];
...
echo '<div><h2 class="mt-5 text-right mb-5">قائمة الحضور المتأخرة</h2></div> ' ;
echo '</div>' ;
...
echo '<tr>' ;
echo '<th class="" style="color:#a17e22;">Worker id</th>' ;
echo '<th class="" style="color:#a17e22;">Worker Name</th>' ;
echo '<th class="" style="color:#a17e22;">Login Time</th>' ;
echo '</tr>' ;
echo '<tr>' ;
