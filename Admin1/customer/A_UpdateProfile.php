...
// check if inputed id is exist :
    $ifsql = "IF EXISTS (SELECT w_id FROM worker WHERE w_id = '$id') print'exist' else print'no'";
...
*/
    ...
    // display hat columns are updated :
if (empty($what)) {
    echo "<div class='row'><div class='p-5 mt-5 col-9 mx-auto text-center h3 text-danger' style='background:rgb(221, 219, 219)'>الرجاء ملء البيانات أعلاه لتحديث بيانات العميل</div></div>";
} else {
   $whatdone = implode(', ',$what);
   echo "<div class='row'><div class='p-5 mt-5 col-9 mx-auto text-center h3' style='background:rgb(221, 219, 219)'>: تم تعديل بيانات العميل الآتية <br><br>$whatdone</div></div>"."<br>";
   echo "</div>";
}
