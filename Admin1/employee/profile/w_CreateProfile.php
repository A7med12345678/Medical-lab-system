        ...
        <div class="p-5 mt-5"
            ...
            <form action="<?php echo $_SERVER['PHP_SELF'];?>">
                <div class="form-group text-right">
                    ...
                    <input type="text" class="form-control" placeholder="" name="w_name" required>
                </div>
                ...
                <div class="form-group text-right">
                    ...
                    <input type="text" class="form-control" placeholder="" name="w_address" required>
                </div>
                <div class="form-group text-right">
                    <label for="text">سن الموظف</label>
                    <input type="text" pattern="[0-9]{2}" class="form-control" placeholder="" name="w_age" required>
                </div>
                
                <div class="form-group text-right">
                    <label for="text">رقم الموظف القومي</label>
                    <input type="text" pattern="[0-9]{14}" class="form-control" placeholder="" name="w_ssin" required>
                </div>
                <div class="form-group text-right">
                    <label for="text">بريد الموظف الالكتروني</label>
                    <input type="text" class="form-control" name="w_mail" required>
                </div>
                <div class="form-group text-right">
                    <label for="text">رقم تليفون الموظف</label>
                    <input type="text" pattern="[0-9]{11}" class="form-control" placeholder="" name="w_phone">
                </div>
                <div class="form-group text-right">
                    <label for="text">راتب الموظف</label>
                    <input type="text" pattern="[0-9]{4}" class="form-control" placeholder="" name="w_salary">
                </div>
                <div class="form-group text-right">
                    <label for="text">صلاحية الموظف</label>
                    <input type="text" pattern="[0-2]{1}" class="form-control" placeholder="" name="w_access">
                </div>
...
$w_access=$_REQUEST['w_access'];
$w_ssin=$_REQUEST['w_ssin'];
$w_mail=$_REQUEST['w_mail'];
$w_salary=$_REQUEST['w_salary'];
$w_insert_date = date("Y-m-d h:i:s");

// count workers numbers (current worker = workers numbers +1)
$sql = "select count(w_id) from worker;";
$stmt = sqlsrv_query( $conn, $sql, $params);
$row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC);
//create worker id :
$count = $row['count(w_id)'];
$fnum = $count + 1 ;
$year = date("Y");
$w_id = $year . $fnum;
//echo ($w_id);
...
//create password :
$pass1 = substr($w_ssin,0,4);
$w_pass = $w_name . "." . $pass1;

    
//insert worker into row :
$sql2 = "INSERT INTO worker (w_id, w_name, w_address, w_age, w_gender, w_phone, w_access, w_ssin, w_salary, w_insert_date, w_mail, w_pass) VALUES ('$w_id', '$w_name', '$w_address', '$w_age', '$w_gender', '$w_phone', '$w_access', '$w_ssin', '$w_salary', '$w_insert_date', '$w_mail', '$w_pass');";
$stmt2 = sqlsrv_query($conn, $sql2, $params);

    if (sqlsrv_rows_affected($stmt2)) {
        echo "<div class='row'><div class='p-5 mt-5 col-9 mx-auto text-center h3 text-danger' style='background:rgb(221, 219, 219)'>تم إضافة الموظف بنجاح</div></div>";
